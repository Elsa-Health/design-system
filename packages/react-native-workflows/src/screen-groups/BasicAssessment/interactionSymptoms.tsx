...
import { useSymptomAssessment } from '../../helpers/context/assessment';
import { getAssocSymptomRecords } from '../../helpers/associated_symptoms';
...
export const ModalComponent = ({ lang }: { lang: Language }) => {
  ...
  React.useEffect(() => {
    ...
    symptoms.forEach(({ symptom: so, data, state }) => {
      const present =
        state !== 'present' ? (state === 'absent' ? false : undefined) : true;
    ...
    >
      ...
      <BottomSheetScrollView style={styles.container}>
        ...
        <RevealContent show={associatedSymptoms.length > 0}>
          ...
          {associatedSymptoms
            .filter((s) => !mainSypmtoms.map((d) => d.id).includes(s))
        ...
        <View style={{ paddingHorizontal: 24 }}>
          {donparItems.map((item) => (
            <DonparItemOption
              {...item}
              key={item.title}
              setData={itemStateUpdate(item.title, item.type === 'multiple')}
            />
          ))}
        </View>
      </RevealContent>
          ...
          {ready && state === 'present' ? (
            ...
            <>
              ...
              >
                ...
                <Text font="medium" style={{ color: '#FFF', marginLeft: 8 }}>
                  {state === 'present' ? tc`actions.save` : tc`actions.present`}
                </Text>
              </Pressable>
            </>
          )}
...
function AssociativeSymptomSection({
  ...
  const { getSymptomById } = useSymptomLocale();
  const content = React.useMemo(() => getSymptomById(id), [getSymptomById, id]);
                  ...
                  onPress={() => {
                    if (!selected) {
                      ...
                      if (!multiple) {
                        ...
                        setData((p) => {
                          const d = [...(p as string[])];
                          const ix = d.findIndex((s) => s === item.id);
                          d.splice(ix, 1);
                          return d;
                        });
                      }
